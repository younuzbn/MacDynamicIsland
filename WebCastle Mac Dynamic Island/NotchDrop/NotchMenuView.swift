...
struct NotchMenuView: View {
    ...
    var donate: some View {
        ColorButton(
            color: ColorfulPreset.colorful.colors,
            image: Image(systemName: "s.circle.fill"),
            title: "Open Skype"
        )
        .onTapGesture {
            openSkype()
    ...
    func openSkype() {
        if let skypeURL = URL(string: "skype:") {  // Use Skype URL scheme
            if NSWorkspace.shared.open(skypeURL) {
                print("Skype opened successfully.")
            } else {
                print("Failed to open Skype.")
            }
        }
    }
